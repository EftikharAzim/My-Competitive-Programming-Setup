...
bool has_bit(ll mask, int i) { return (mask >> i) & 1; }
ll set_bit(ll mask, int i) { return mask | (1LL << i); }
ll clear_bit(ll mask, int i) { return mask & ~(1LL << i); }
ll flip_bit(ll mask, int i) { return mask ^ (1LL << i); }
...
inline ll mod_inv(ll a) { return mod_pow(a, MOD - 2); }
...
bool ge(double a, double b) { return a + EPS > b; }
...
template <typename T>
T ceil_div(T a, T b) { return (a + b - 1) / b; }

// Binary search templates
template <typename T, typename F>
T binary_search_first_true(T lo, T hi, F &&pred)
{
    while (lo < hi)
    {
        T mid = lo + (hi - lo) / 2;
...
template <typename T, typename F>
T binary_search_last_true(T lo, T hi, F &&pred)
{
    while (lo < hi)
    {
        T mid = lo + (hi - lo + 1) / 2;
